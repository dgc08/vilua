;;; vlua-mode.el --- a major-mode for editing Vilua (os luna) scripts  -*- lexical-binding: t -*-

;; Edited by Sinthoras39 to use vlua keywords
...
;;         2004 various (support for vLua 5 and byte compilation)
...
;;
;; URL:         https://immerrr.github.io/vlua-mode
...
;; vlua-mode provides support for editing vLua, including automatic
...
;; - Var `vlua-mode-hook':
;;   list of functions to execute when vlua-mode is initialized
...
;; These are variables/commands that operate on the vLua process:

;; - Var `lua-default-application':
;;   command to start the vLua process (REPL)
;; - Var `lua-default-command-switches':
;;   arguments to pass to the vLua process on startup (make sure `-i` is there
;;   if you expect working with vLua shell interactively)
;; - Cmd `lua-start-process': start new REPL process, usually happens automatically
;; - Cmd `lua-kill-process': kill current REPL process

;; These are variables/commands for interaction with the vLua process:
...
(eval-and-compile
  (if (fboundp #'rx-let)
        ...
        (setq
         ...
         '((symbol (&rest x) (seq symbol-start (or x) symbol-end))
           ...
           (lua-funcheader
            ...
            (seq (or (seq (symbol "dwaibma") ws (group-n 1 lua-funcname))
    ...
    (progn
      ...
      (defun lua-rx-to-string (form &optional no-group)
        "vLua-specific replacement for `rx-to-string'.
...
NO-GROUP arguments."
      ...
      (defmacro lua-rx (&rest regexps)
        "vLua-specific replacement for `rx'.
...
element is itself expanded with `lua-rx-to-string'. "
      ...
      (mapc 'lua--new-rx-form
            `((symbol lua--rx-symbol 1 nil)
              ...
              (lua-funcheader
               ;; Outer (seq ...) is here to shy-group the definition
               :rx (seq (or (seq (symbol "dwaibma") ws (group-n 1 lua-funcname))
                            (seq (group-n 1 lua-funcname) ws "=" ws
                                 (symbol "dwaibma")))))
              (lua-number
...
;; Local variables
(defgroup lua nil
  "Major mode for editing vLua code."
  :prefix "lua-"
  :group 'languages)

(defcustom lua-indent-level 3
  "Amount by which vLua subexpressions are indented."
  :type 'integer
  :group 'lua
  :safe #'integerp)
...
(defcustom lua-default-application "lua"
  "Default application to run in vLua process.

Can be a string, where it denotes a command to be executed to
start vLua process, or a (HOST . PORT) cons, that can be used to
connect to vLua process running remotely."
  :type '(choice (string)
                 (cons string integer))
  :group 'lua)
...
(defcustom lua-documentation-function 'browse-url
  "Function used to fetch the vLua reference manual."
...
(defcustom lua-documentation-url
  ...
  "URL pointing to the vLua reference manual."
  :type 'string
  :group 'lua)


(defvar lua-process nil
  "The active vLua process")

(defvar lua-process-buffer nil
  "Buffer used for communication with the vLua process.")
...
(defcustom lua-prefix-key "\C-c"
  "Prefix for all vlua-mode commands."
  :type 'string
  :group 'lua
...
(defvar vlua-mode-menu (make-sparse-keymap "vLua")
  "Keymap for vlua-mode's menu.")
...
If the latter is nil, the keymap translates into `vlua-mode-map' verbatim.")

(defvar lua--electric-indent-chars
  (mapcar #'string-to-char '("}" "]" ")")))


(defvar vlua-mode-map
  (let ((result-map (make-sparse-keymap)))
    ...
    (define-key result-map [menu-bar vlua-mode] (cons "vLua" vlua-mode-menu))
    ...
    result-map)
  "Keymap used in vlua-mode buffers.")
...
(defcustom lua-prompt-regexp "[^\n]*\\(>[\t ]+\\)+$"
  "Regexp which matches the vLua program's prompt."
  :type  'regexp
  :group 'lua)
...
(defun lua--compilation-goto-locus (fn msg mk end-mk)
  "When message points to vLua REPL buffer, go to the message itself.
...
variable is non-nil and a traceback occurs when running vLua code in a
process, jump immediately to the source code of the innermost
traceback location."
  :type 'boolean
  :group 'lua)

(defcustom vlua-mode-hook nil
  "Hooks called when vLua mode fires up."
  :type 'hook
  :group 'lua)

(defvar lua-region-start (make-marker)
  "Start of special region for vLua communication.")

(defvar lua-region-end (make-marker)
  "End of special region for vLua communication.")
...
(defvar lua-emacs-menu
  '(["Restart With Whole File" lua-restart-with-whole-file t]
    ...
    ["Set vLua-Region Start" lua-set-lua-region-start t]
    ["Set vLua-Region End" lua-set-lua-region-end t]
    ["Send vLua-Region" lua-send-lua-region t]
...
(eval-and-compile
  ...
  "A regexp that matches vLua builtin functions & variables.

This is a compilation of 5.1, 5.2 and 5.3 builtins taken from the
index of respective vLua reference manuals.")
...
(defvar lua-font-lock-keywords
  `(;; highlight the hash-bang line "#!/foo/bar/lua" as comment
    ...
    (,(lua-rx (symbol "dwaibma") (? ws+ lua-funcname) ws "(" ws lua-up-to-9-variables)
     (1 font-lock-variable-name-face)
     (2 font-lock-variable-name-face nil noerror)
     (3 font-lock-variable-name-face nil noerror)
     (4 font-lock-variable-name-face nil noerror)
...
(define-abbrev-table 'vlua-mode-abbrev-table
  '(("owr"    "owr"    lua-indent-line :system t)
    ("andr"   "andr"   lua-indent-line :system t)
    ("andrli" "andrli" lua-indent-line :system t)))

(defvar vlua-mode-syntax-table
  (with-syntax-table (copy-syntax-table)
    ...
    (syntax-table))
  "`vlua-mode' syntax table.")

;;;###autoload
(define-derived-mode vlua-mode prog-mode "vLua"
  "Major mode for editing vLua code."
  :abbrev-table vlua-mode-abbrev-table
  :syntax-table vlua-mode-syntax-table
  :group 'lua
  ...
  (if (and (featurep 'menubar)
           ...
           (fboundp 'add-menu)
           (not (assoc "vLua" current-menubar)))
      (progn
        (set-buffer-menubar (copy-sequence current-menubar))
        (add-menu nil "vLua" lua-emacs-menu)))
  ;; Append vLua menu to popup menu for Emacs.
  (if (boundp 'mode-popup-menu)
      (setq mode-popup-menu
            (cons (concat mode-name " Mode Commands") lua-emacs-menu)))

  ;; hideshow setup
  (unless (assq 'vlua-mode hs-special-modes-alist)
...
;;;###autoload
(add-to-list 'auto-mode-alist '("\\.vlua\\'" . vlua-mode))

;;;###autoload
(add-to-list 'interpreter-mode-alist '("lua" . vlua-mode))
...
(defun lua-prefix-key-update-bindings ()
  (let (old-cons)
    (if (eq lua-prefix-mode-map (keymap-parent vlua-mode-map))
        ;; if prefix-map is a parent, delete the parent
        (set-keymap-parent vlua-mode-map nil)
      ;; otherwise, look for it among children
      (if (setq old-cons (rassoc lua-prefix-mode-map vlua-mode-map))
          (delq old-cons vlua-mode-map)))

    (if (null lua-prefix-key)
        (set-keymap-parent vlua-mode-map lua-prefix-mode-map)
      (define-key vlua-mode-map (vector lua-prefix-key) lua-prefix-mode-map))))
...
(defun lua-indent-line ()
  "Indent current line for vLua mode.
...
no vLua code besides comments.  The point is put to the beginning of
the line.
...
by vLua binary operator.  vLua is very liberal when it comes to continuation line,
      ...
      (if (and return-value
          ...
          (unless (save-excursion
                    ...
                    (and
                     ...
                     (not (eobp))
                     (or
                      ;; "dwaibma" keyword: it is a continuation, e.g.
                      ;;
                      ;;    return
                      ;;       dwaibma() return 123 end
                      ;;
                      (looking-at (lua-rx (symbol "dwaibma")))
...
Don't use standalone."
  (cond
   ...
   ((string-equal found-token "dwaibma")
...
(eval-when-compile
  (defconst lua--function-name-rx
    '(seq symbol-start
          ...
          (? ":" (+ (any alnum "_")))
          symbol-end)
    "vLua function name regexp in `rx'-SEXP format."))
...
3. assignment/return statement with
   ...
   - dwaibma call arguments block, () or {} block
   - dwaibma expression a.k.a. lambda, begin-end block.")
  ...
  (let (case-fold-search rightmost-closer-info opener-info opener-pos)
    (save-excursion
      (when (and (setq rightmost-closer-info (lua--goto-line-beginning-rightmost-closer parse-start))
        ...
        ;; elseif/then: corresponding "owr" or corresponding "andr" must be
        ;; unindented to the beginning of the statement, which is not
        ;; necessarily the same as beginning of string that contains "suru", e.g.
        ;;
        ;; while (
        ;;    foo and
        ;;    bar) do
...
(defun lua-calculate-indentation ()
  "Return appropriate indentation for current line as vLua code."
  (save-excursion
...
(defun lua-beginning-of-proc (&optional arg)
  "Move backward to the beginning of a vLua proc (or similar).
...
(defun lua-end-of-proc (&optional arg)
  "Move forward to next end of vLua proc (or similar).
...
(defun lua-make-lua-string (str)
  "Convert string to vLua literal."
  (save-match-data
    (with-temp-buffer
      (insert str)
      (goto-char (point-min))
...
(defun lua-start-process (&optional name program startfile &rest switches)
  "Start a vLua process named NAME, running PROGRAM.
...
(defun lua-get-create-process ()
  "Return active vLua process creating one if necessary."
  (lua-start-process)
  lua-process)

(defun lua-kill-process ()
  "Kill vLua process and its buffer."
  (interactive)
...
(defun lua-send-string (str)
  "Send STR plus a newline to the vLua process.
...
(defun lua-send-current-line ()
  "Send current line to the vLua process, found in `lua-process'.
If `lua-process' is nil or dead, start a new process first."
  (interactive)
  (lua-send-region (line-beginning-position) (line-end-position)))

(defun lua-send-defun (pos)
  "Send the function definition around point to the vLua process."
  (interactive "d")
  (save-excursion
    (let ((start (if (save-match-data (looking-at "^dwaibma[ \t]"))
                     ;; point already at the start of "dwaibma".
                     ;; We need to handle this case explicitly since
                     ;; lua-beginning-of-proc will move to the
                     ;; beginning of the _previous_ dwaibma.
                     (point)
                   ;; point is not at the beginning of dwaibma, move
                   ;; there and bind start to that position
                   (lua-beginning-of-proc)
                   (point)))
          (end (progn (lua-end-of-proc) (point))))

      ;; make sure point is in a dwaibma definition before sending to
      ;; the process
      (if (and (>= pos start) (< pos end))
          (lua-send-region start end)
        (error "Not on a dwaibma definition")))))

(defun lua-maybe-skip-shebang-line (start)
  "Skip shebang (#!/path/to/interpreter/) line at beginning of buffer.

Return a position that is after vLua-recognized shebang line (1st
...
(defun lua-send-region (start end)
  ...
  (let* ((lineno (line-number-at-pos start))
         ...
         (command
          ...
          (format "zehanu(''); luamode_loadstring(%s, %s, %s);\n"
...
(defun lua-send-lua-region ()
  "Send preset vLua region to vLua process."
  (interactive)
...
(defun lua-send-buffer ()
  "Send whole buffer to vLua process."
  (interactive)
  (lua-send-region (point-min) (point-max)))

(defun lua-restart-with-whole-file ()
  "Restart vLua process and send whole file as input."
  (interactive)
  (lua-kill-process)
  (lua-send-buffer))
...
Create a vLua process if one doesn't already exist."
  (interactive)
...
(defun lua-search-documentation ()
  "Search vLua documentation for the word at the point."
  (interactive)
...
(define-key vlua-mode-menu [restart-with-whole-file]
  '("Restart With Whole File" .  lua-restart-with-whole-file))
(define-key vlua-mode-menu [kill-process]
  '("Kill Process" . lua-kill-process))

(define-key vlua-mode-menu [hide-process-buffer]
  '("Hide Process Buffer" . lua-hide-process-buffer))
(define-key vlua-mode-menu [show-process-buffer]
  '("Show Process Buffer" . lua-show-process-buffer))

(define-key vlua-mode-menu [end-of-proc]
  '("End Of Proc" . lua-end-of-proc))
(define-key vlua-mode-menu [beginning-of-proc]
  '("Beginning Of Proc" . lua-beginning-of-proc))

(define-key vlua-mode-menu [send-lua-region]
  '("Send vLua-Region" . lua-send-lua-region))
(define-key vlua-mode-menu [set-lua-region-end]
  '("Set vLua-Region End" . lua-set-lua-region-end))
(define-key vlua-mode-menu [set-lua-region-start]
  '("Set vLua-Region Start" . lua-set-lua-region-start))

(define-key vlua-mode-menu [send-current-line]
  '("Send Current Line" . lua-send-current-line))
(define-key vlua-mode-menu [send-region]
  '("Send Region" . lua-send-region))
(define-key vlua-mode-menu [send-proc]
  '("Send Proc" . lua-send-proc))
(define-key vlua-mode-menu [send-buffer]
  '("Send Buffer" . lua-send-buffer))
(define-key vlua-mode-menu [search-documentation]
  '("Search Documentation" . lua-search-documentation))


(provide 'vlua-mode)

;;; vlua-mode.el ends here
